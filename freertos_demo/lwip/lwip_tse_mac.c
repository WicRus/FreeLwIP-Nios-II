...
int tse_mac_init(int iface, struct ethernetif *ethernetif)
{
        ...
        if (sys_sem_valid(&tse[iface].rx_semaphore)==0) {
                dprintf(("[LwIP] creating RX SGDMA semaphore\n"));
        ...
        dprintf(("[LwIP] tse_mac_init iface %d\n", iface));
        ...
        if (!sgdma_tx_dev)
        {
                dprintf(("[LwIP] Error opening TX SGDMA\n"));
                return ENP_RESOURCE;
        }
        ...
        if (!sgdma_rx_dev)
        {
                dprintf(("[LwIP] Error opening RX SGDMA\n"));
                return ENP_RESOURCE;
        }
        ...
        if (x >= 10000)
                dprintf(("[LwIP] TSEMAC SW reset bit never cleared!\n"));

        dat = IORD_ALTERA_TSEMAC_CMD_CONFIG(tse[iface].mi.base);
        if ( (dat & 0x03) != 0 )
                dprintf(("[LwIP] WARN: RX/TX not disabled after reset... missing PHY clock? CMD_CONFIG=0x%08x\n", dat));
        else
                dprintf(("[LwIP] OK, x=%d, CMD_CONFIG=0x%08x\n", x, dat));
        ...
        {
                dprintf(("[LwIP] Error: Incompatible %d value with TX_CMD_STAT register return TxShift16 value. \n",ETH_PAD_SIZE));
                return ERR_IF;
        }
        ...
        {
                dprintf(("[LwIP] Error: Incompatible %d value with RX_CMD_STAT register return RxShift16 value. \n",ETH_PAD_SIZE));
                return ERR_IF;
        }
        ...
        dprintf(("[LwIP] MAC post-initialization: CMD_CONFIG=0x%08x\n",
        ...
        ethernetif->link_alive = alt_tse_phy_rd_mdio_reg(pmac_info->pphy_info, TSE_PHY_MDIO_STATUS, TSE_PHY_MDIO_STATUS_LINK_STATUS, 1) != 0;// && (((IORD(&(mi_base->mdio1.reg10), 0) >> 4) & 0xF) <= 3);
...
int tse_sgdma_read_init(lwip_tse_info* tse_ptr)
{
        ...
        dprintf(("[LwIP] RX descriptor chain desc (%d depth) created\n",0));
